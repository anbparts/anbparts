...
export const api = {
  motos: {
    ...
    replaceAnexo: (id: number, key: string, anexo: { name: string; dataUrl: string }) => req<any>(`/motos/${id}/anexos/${encodeURIComponent(key)}`, { method: 'PATCH', body: JSON.stringify(anexo) }),
  ...
  detran: {
    ...
    saveConfig:       (_data: any) => Promise.resolve({}),
    execucoes:        (_params?: any) => Promise.resolve({ execucoes: [] }),
    execucao:         (_id: number) => Promise.resolve(null),
    artifactUrl:      (_id: number, _kind: string, _index?: number) => '',
    createExecucao:   (_data: any) => Promise.resolve({}),
    updateEtapa:      (_id: number, _step: string, _data: any) => Promise.resolve({}),
    finalizarExecucao:(_id: number, _data: any) => Promise.resolve({}),
    deleteExecucao:   (_id: number) => Promise.resolve({}),
  },
  mercadoLivre: {
