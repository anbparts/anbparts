...
const s: any = {
  ...
  input:  { width: '100%', background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 7, padding: '9px 13px', fontSize: 14, fontFamily: 'Inter, sans-serif', outline: 'none', color: 'var(--gray-800)' },
  ...
  stepN:  { width: 26, height: 26, borderRadius: '50%', background: 'var(--blue-500)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0, marginTop: 1 },
};
...
export default function BlingConfigPage() {
  ...
  async function load() {
    setLoading(true);
    try { setConfig(await fetch(`${API}/bling/config`).then(r => r.json())); }
  ...
  async function saveCredentials() {
    ...
    try {
      ...
      setClientId(''); setClientSecret(''); await load();
    } catch { alert('Erro ao salvar'); }
    setSaving(false);
  }

  async function connectBling() {
    const d = await fetch(`${API}/bling/auth-url`).then(r => r.json());
  ...
  async function testConn() {
    ...
    try { setConnStatus(await fetch(`${API}/bling/status`).then(r => r.json())); }
    catch { setConnStatus({ ok: false, error: 'Sem resposta' }); }
  }
  ...
  if (loading) return (
    <>
      <div style={s.topbar}><div style={{ fontSize: 17, fontWeight: 600, color: 'var(--gray-800)' }}>Configuração Bling</div></div>
  ...
  return (
    <>
      <div style={s.topbar}>
        <div>
          <div style={{ fontSize: 17, fontWeight: 600, color: 'var(--gray-800)', letterSpacing: '-0.3px' }}>Configuração Bling</div>
          <div style={{ fontSize: 12, color: 'var(--gray-400)', marginTop: 2 }}>Credenciais OAuth e conexão</div>
        </div>
        <div>
          {connected
            ? <span style={{ ...s.badge, background: 'var(--green-light)', color: 'var(--green)', border: '1px solid #86efac' }}>✓ Bling conectado</span>
            : config?.clientId
              ? <span style={{ ...s.badge, background: 'var(--amber-light)', color: 'var(--amber)', border: '1px solid #fcd34d' }}>⚠ Aguardando autorização</span>
              : <span style={{ ...s.badge, background: 'var(--gray-100)', color: 'var(--gray-400)', border: '1px solid var(--border)' }}>○ Não configurado</span>
          }
        </div>
      </div>
      ...
      <div style={{ padding: 28, maxWidth: 680 }}>
        ...
        <div style={s.card}>
          ...
          {[
            { n: 1, t: <>No Bling: <strong>⚙ Preferências → Todas as Configurações → Cadastro de Aplicativos → + CRIAR NOVO</strong></> },
            ...
            { n: 3, t: <>Escopos: marque <strong>Produtos</strong> (leitura) e <strong>Pedidos de Venda</strong> (leitura). Salve.</> },
            { n: 4, t: <>Copie o <strong>Client ID</strong> e <strong>Client Secret</strong> e cole abaixo.</> },
          ].map(({ n, t }) => (
            <div key={n} style={s.step}><div style={s.stepN}>{n}</div><div style={{ fontSize: 13.5, color: 'var(--gray-600)', lineHeight: 1.7 }}>{t}</div></div>
          ))}
        </div>
        ...
        <div style={s.card}>
          ...
          {config?.clientId && (
            <div style={{ background: 'var(--green-light)', border: '1px solid #86efac', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 13, color: 'var(--green)' }}>
              ...
              {config.connectedAt && <span style={{ marginLeft: 8, color: 'var(--gray-400)', fontSize: 12 }}>· {new Date(config.connectedAt).toLocaleDateString('pt-BR')}</span>}
            </div>
          )}
          ...
          <div style={s.card}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {!connected
                ...
                : <>
                    <button style={{ ...s.btn, background: 'var(--green-light)', color: 'var(--green)', border: '1px solid #86efac' }} onClick={testConn}>{connStatus?.loading ? '⏳...' : '✓ Testar conexão'}</button>
                    <button style={{ ...s.btn, background: 'var(--red-light)', color: 'var(--red)', border: '1px solid #fca5a5' }} onClick={disconnect}>Desconectar</button>
                  </>
              }
            </div>
            {connStatus && !connStatus.loading && (
              <div style={{ marginTop: 14, padding: '10px 14px', background: connStatus.ok ? 'var(--green-light)' : 'var(--red-light)', border: `1px solid ${connStatus.ok ? '#86efac' : '#fca5a5'}`, borderRadius: 8, fontSize: 13, color: connStatus.ok ? 'var(--green)' : 'var(--red)' }}>
                {connStatus.ok ? '✓ Conexão OK' : `✗ Erro: ${connStatus.error}`}
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ ...s.card, background: 'var(--gray-50)' }}>
          <div style={{ fontSize: 12, color: 'var(--gray-400)', lineHeight: 1.9 }}>
            • URL de callback no Bling: <code style={{ fontFamily: 'JetBrains Mono, monospace', background: 'var(--gray-200)', padding: '0 4px', borderRadius: 3 }}>{API}/bling/callback</code><br/>
            • Configure o de/para de SKU em <a href="/bling/config-produtos" style={{ color: 'var(--blue-500)' }}>Config. Produtos</a> antes de importar
          </div>
        </div>
      </div>
    </>
